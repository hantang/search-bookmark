...
function updateTableCount(table = null, cnt = null) {
  ...
  if (cnt && cnt > 0) {
    ...
    if (table == null)
      table = document.getElementById(bookmarkTableStr);
    if (table) {
...
function initSearchConditions(columns) {
  ...
  const checkboxes = createRadioOrCheckbox(searchConditionOption, searchCheckClassName, false, 'name-checkbox-input-condition')
  part0.appendChild(checkboxes);
  fieldset1.appendChild(part0);

  const part1 = document.createElement("div");
  part1.className = className;
  const logicRadio = createRadioOrCheckbox(searchLogicOptions, searchRadioClassName, true, 'name-radio-input-logic')
  ...
  columns.forEach((label, index) => {
    ...
    const radioElement = createRadioOrCheckbox(searchFilterOptions, searchRadioClassName, true, `name-radio-input-filter-${index}`)
...
function filterTableData() {
  ...
  const conditions = Array.from(conditionsRaw).filter(cond => cond.querySelector(".search-condition-input")
  ...
  for (let i = start; i < rows.length; i++) {
    ...
    if ((isLogicAnd && matchCount === conditions.length) ||
      (!isLogicAnd && matchCount > 0)) {
      displayRow = true;
    }
...
function resetSearchConditions() {
  ...
  const firstRadioButtons = document.querySelectorAll(`.${className} input[type="radio"]:first-child`);
